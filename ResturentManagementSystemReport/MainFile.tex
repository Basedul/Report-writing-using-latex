...
\usepackage[paperheight=11.69in,paperwidth=8.27in,includefoot,top=3cm, bottom=3cm, left=3cm, right=3cm]{geometry}
...
\chead{%
  \center Shenyang Aerospace University Graduation Design(Thesis)
}
\cfoot{\thepage}
...
\AtBeginDocument{
  \addtocontents{toc}{\fontsize{12}{19}\fontfamily{ptm}\selectfont}
  \addtocontents{lof}{\fontsize{12}{19}\fontfamily{ptm}\selectfont}
  \addtocontents{lot}{\fontsize{12}{19}\fontfamily{ptm}\selectfont}
}
\usepackage{titlesec}
...
\RaggedRight{\bfseries Key Words: } Asian Restaurant Management Application, Java technique, Wamp server, MySQL, Database.
...
the main objectives that the application expects to achieve and a brief introduction to existing solutions.\\It is known globally that, in today's market, it is extremely difficult to start a new small-scale business and live-through the competition from the well-established and settled owners. In fast paced time of today, when everyone is squeezed for time, the majority of people are finicky when it comes to placing a food order. The customers of today are not only attracted because placing an order online is very convenient but also because they have visibility into the items offered, price and extremely simplified navigation for the order.\\Ordering system that I am proposing here, greatly simplifies the ordering process for both the clients and the restaurant. Application presents an interactive and up-to-date menu with all available options in an easy to use manner. Clients can choose one or more items to place an order. Clients can view all the order details before checking out. At the end, Clients gets order confirmation details. Once the order is placed it is entered in the database and retrieved in pretty much real time. This allows Restaurant Employees to quickly go through the orders as they are received and process all orders efficiently and effectively with minimal delays and confusion.
}	
...
%{\bfseries \Large Technological Background} 
	...
	\tab{The tools used to accomplish in this project are MySQL\cite{Ref:5}, Apache Server\cite{Ref:8} and Java\cite{Ref:7} is Object oriented programming language .}
...
gather, the asian restaurant management system will be designed as below:}
	...
	\tab{As shown Fig\ref{fig:archi} is showing the \textbf{Asian Restaurant Management System} is divided into some modules and submodules.}
	\newpage
...
\textbf{Related Tables} The relationship line indicates that a foreign-key relationship exists between one table and another. For a one-to-many relationship, the foreign-key table is the table near the line's figure-eight symbol. If both endpoints of the line attach to the same table, the relationship is a reflexive relationship.
	...
	\tab As shown in Fig\ref{fig:adminreg} if admin wants to check the order list from an client and add new cuisine items to a foodstuff, admin must first signup with name, user name, email and password. After registration, he will be able to check the order list after signin, else he can not even see the order list and he will not be able to add cuisine to the food list . 
	\begin{figure}[H]
		\centering
		\includegraphics[height=2.5in]{/home/basedul/Documents/Report-writing-using-latex/ResturentManagementSystemReport/adminent.png}
		\caption{\hspace{0.35em}Admin entity diagram}
		\label{fig:adminreg} 
	\end{figure}\tab{As shown in figure \ref{fig:clientreg} client can browse about cuisines name, price and food details from food list. But if client wants to place an order by confirming, client must need to sign up first with client name, username, e-mail and password. And then signin with his username and password. After logging in, he can order the food from the cuisine catagory. After ordering, client can be able to signout.}
	\begin{figure}[H]
		\centering
		...
		\label{fig:clientreg} 
	\end{figure}\newpage
	\tab{As shown in Fig\ref{fig:foodadd} if admin wants to add new cuisine items to a foodstuff/order menu, admin must first signup with name, user name, email and password. After registration, he will be able to check the order list after signin, else he can not even see the order list and he will not be able to add cuisine to the food list.}
	\begin{figure}[H]
		\centering
		\includegraphics[height=2.5in]{/home/basedul/Documents/Report-writing-using-latex/ResturentManagementSystemReport/fooddet.png}
		\caption{\hspace{0.35em}Food add entity diagram}
		\label{fig:foodadd} 
	\end{figure}
	
	\tab{As shown in Fig\ref{fig:commentsent} after a placing an order or requesting for home delivery, client can comment on each cuisine items. And also can give scores for a specific item of cuisine. If you want to comment or score, you must signin , write the name of the cuisine in box then write the comment and select the score and post it.} 
	\begin{figure}[H]
		\centering
		\includegraphics[height=2.5in]{/home/basedul/Documents/Report-writing-using-latex/ResturentManagementSystemReport/commtent.png}
		\caption{\hspace{0.35em}Comments/Score entity diagram}
		\label{fig:commentsent} 
	\end{figure}
	\subsubsection{E-R diagram}
		\tab An entity relationship diagram (ERD) \cite{Ref:20}, also known as an entity relationship model, is a graphical representation of an information system that depicts the relationships among people, objects, places, concepts or events within that system. An ERD is a data modeling technique that can help define business processes and be used as the foundation for a relational database.\\\tab Entity relationship diagrams provide a visual starting point for database design that can also be used to help determine information system requirements throughout an organization. After a relational database is rolled out, an ERD can still serve as a referral point, should any debugging or business process re-engineering be needed later.\\\tab However, while an ERD can be useful for organizing data that can be represented by a relational structure, it can't sufficiently represent semi-structured or unstructured. It's also unlikely to be helpful on its own in integrating data into a preexisting information system.
...
%	\end{lstlisting}
	...
	\tab The database structure is the collection of record type and field type definitions that comprise your database. These define the type of entities or research objects you wish to capture (e.g. Person) Fields. These are the properties or attributes that describe your record types (e.g. Gender, Age, Height etc).\\
		...
		\begin{table}[H]
		\center
	\caption{\hspace{0.4em}Registration for admin}
	...
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{u-u-name}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{varchar(80)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{ }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{Stores the admin user name}} &\\[15pt]
	\hline
...
%	\hline
	...
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores the client user name}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {5}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {u-mail}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(50)}} & 
	...
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores the name of foods}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {3}} & 
	...
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores the client name who is order foods}} &\\[15pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	...
	\begin{table}[H]
		\center
	\caption{\hspace{0.4em}Top foods}
	\label{tab:topfoods}
	\begin{tabular}{M{0.6cm}|M{1.5cm}|M{2cm}|M{1cm}|M{1cm}|M{2.5cm}|M{3.2cm}N}
	\specialrule{.15em}{.05em}{.05em}
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{No}} & 
	...
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{1}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {tf-id}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {int(3)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {No}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Auto-increment}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores the top foods id}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {2}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {tf-name}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores the name of top foods}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {3}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {tf-score}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {float(10, 2)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores the score}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {4}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {tf-comments}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores comments}} &\\[15pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	...
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{1}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{sl-id}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{int(3)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {{No}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Auto-increment}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores salads id}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {2}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {sl-name}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores salad name}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {3}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {sl-price}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {float(10, 2)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores price of salads}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {4}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {sl-details}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores salads details}} &\\[15pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	...
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {1}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {ck-id}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {int(3)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {No}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Auto-increment}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores chickens id}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {2}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {ck-name}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize{10}{8}\fontfamily{ptm}\selectfont { {Stores chickens name}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {3}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {ck-price}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {float(10, 2)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores price of chickens}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {4}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {ck-details}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize{10}{8}\fontfamily{ptm}\selectfont { {Stores chickens details}} &\\[15pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	...
	\begin{table}[H]
		\center
	\caption{\hspace{0.4em}Vegetable and Tofu table}
	\label{tab:vegetofu1}
	\begin{tabular}{M{0.6cm}|M{1.5cm}|M{2cm}|M{1cm}|M{1cm}|M{2.5cm}|M{3.2cm}N}
	\specialrule{.15em}{.05em}{.05em}
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{No}} & 
	...
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {1}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {vt-id}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {int(3)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {No}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Auto-increment}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores vegetables and tofu's id}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {2}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {vt-name}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores vegetables and tofu's name}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {3}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {vt-price}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {float(10, 2)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores vegetables and tofu's chickens}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {4}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {vt-details}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores vegetables and tofu's details}} &\\[15pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	...
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{Description}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{1}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {nr-id}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {int(3)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {No}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Auto-increment}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores noddles and rice's id}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {2}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {nr-name}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores noddles and rice's name}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {3}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {nr-price}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {float(10, 2)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores noddles and rice's of chickens}} &\\[15pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {4}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {nr-details}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {varchar(255)}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Yes}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {None}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { { }} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont { {Stores noddles and rice's details}} &\\[15pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	\newpage
...
of the flowchart is to show functions the Asian Restaurant Management Application has.\\\tab{Client can browse about cuisines name, price and food details from food list. But if client wants to place an order by confirming, client must need to sign up first with client name, username, e-mail and password. And then signin with his username and password. After logging in, he can order the food from the cuisine catagory. After confirming an order, client can be able to signout. When an admin wants to check the order list from an client and add new cuisine items to a foodstuff, admin must first signup with name, user name, email and password. After registration, he will be able to check the order list after signin, else he can not even see the order list and he will not be able to add cuisine to the food list. But if client wants to place an order by confirming, client must need to sign up first with client name, username, e-mail and password. And then signin with his username and password. After logging in, he can order the food from the cuisine catagory. After ordering, client can be able to signout . After a placing an order or requesting for home delivery, client can comment on each cuisine items. And also can give scores for a specific item of cuisine. If you want to comment or score, you must signin , write the name of the cuisine in box then write the comment and select the score and post it. And the system will be showing top foods based on the client's score and comment. Client can also be albe to search specific cuisine item by it's item catagory, price and name.}	
		...
		\caption{\hspace{0.35em}System flowchart}
		\label{fig:flowchart} 
	\end{figure}
	\newpage
...
window will be instead.
		\begin{figure}[H]
		\centering
		\includegraphics[height=4.3in]{/home/basedul/Documents/Report-writing-using-latex/ResturentManagementSystemReport/regispag.png}
	...
	\subsubsection{Order list}
		...
		\begin{figure}[H]
		\centering
		\includegraphics[height=4.5in]{/home/basedul/Documents/Report-writing-using-latex/ResturentManagementSystemReport/seeclientorderlist.png}
	...
	\subsubsection{Confirmed order after successfull signin}
		...
		\begin{figure}[H]
		\centering
		\includegraphics[height=4.5in]{/home/basedul/Documents/Report-writing-using-latex/ResturentManagementSystemReport/confirmedOrder.png}
...
%{\bfseries \Large Implementation and Testing}
	...
	\begin{itemize}
		...
		\item installation, configuration and testing of network servers, workstations and peripherals
		\item installation of operating system software on servers and workstations
		\item installation and testing of application software on application servers and workstations
	...
	\subsection{Document list}
		...
		\begin{table}[H]
		\center
	\caption{\hspace{0.4em}Document list}
	\label{tab:vegetofu}
	\begin{tabular}{M{0.5cm}|M{5cm}|M{7.8cm}N}
	\specialrule{.15em}{.05em}{.05em} 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{No}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{Document}} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {\bfseries{Purpose}} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {1} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {DatabaseConnection.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for implementation of database connection with local host using jdbc driver} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {2} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {Appetizers.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add appetizers name, price and details into appetizers table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {3} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {BeefLamb.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add beef or lamb name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {4} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {Chicken.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add chicken name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {5} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {NoddlesRice.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add noddles and rice name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {6} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {Pork.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add pork name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {7} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {Salads.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add salads name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {8} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SeaFood.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add seafood name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {8} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {Soups.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add Soups name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {8} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {VegeTofu.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for add vegetabls and tofu name, price and details into specific table of food} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {9} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {MainActivity.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {MainActivity class use for design and implementation of homepage} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {10} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {OrderInsertIntoOrderList.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {In this class OrderInsertIntoOrderList, order from clients are insert into order list table} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {11} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {PersonalInfoFrame.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class used for modify personal info of client} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {11} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {PersonalInfoFrameAdmin.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class used for modify personal info of admin} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {12} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {ReadFromTable.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for read data from database and set into homepage} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {13} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {WrapTextInJTable.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class used for wraping of data into jtable} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {14} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SignInClass.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for sign in of client} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {15} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SignoutForClient.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for signout to the client} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {16} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SignupClass.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for signup of the client} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {17} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SignInClassForAdmin.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for sign in of Admin} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {18} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SignoutForAdmin.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for signout to the admin} &\\[10pt]
	\hline
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {19} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {SignupClassForAdmin.java} & 
	\fontsize {10}{8}\fontfamily{ptm}\selectfont {This class use for signup of the admin} &\\[10pt]
	\specialrule{.15em}{.05em}{.05em}
	\end{tabular}
	\end{table}
	\newpage
