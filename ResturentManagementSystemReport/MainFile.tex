...
below:}\\
	...
	\tab{The main deliverable of this project is to build a simple and easy use of restaurant management system following the specific software requirements as well as the programming languages.}
\newpage
...
%{\bfseries \Large Technological Background} 
	...
	\tab{Use these three tools are MySQL\cite{Ref:5}, Apache Server\cite{Ref:8} and Java\cite{Ref:7} is Object oriented programming language .}
...
%			RankList(rlid, rlname, rlscore, rlcomments)
		\begin{itemize}
			\item RegistrationForAdmin(uid, uf, ul, uu, um, ufp, usp, status).
			\item Registration(uid, uf, ul, uu, um, ufp, usp, status).
			\item APPTIZERS(appid, appname, appprice, appdetails).
			\item BeefLamb(blid, blname, blprice, bldetails).
			\item Chicken(ckid, ckname, ckprice, ckdetails).
			\item Drink(did, dname, dprice, ddetails).
			\item NoddlesRice(nrid, nrname, nrprice, nrdetails).
			\item Pork(poid, poname, poprice, podetails).
			\item Salad(said, saname, saprice, sadetails).
			\item SeaFood(sfid, sfname, sfprice, sfdetails).
			\item Soups(soid, soname, soprice, sodetails).
			\item VegeTofu(vtid, vtname, vtprice, vtdetails).
			\item ClientOrderList(colid, colname, colprice, colUser).
			\item RankList(rlid, rlname, rlscore, rlcomments).
		\end{itemize}
...
%	\end{lstlisting}
	...
	\tab This table \ref{tab:vegetofu1} is used for hold the list of Vegetables and Tofus.
	\begin{table}[H]
		\center
